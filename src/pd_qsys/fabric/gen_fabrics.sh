...
rm -rf *.qsys apf.tcl bpf.tcl soc_apf.tcl *.qpf *.qsf ip.ipx hw_ip.iipx sw_ip.iipx gen_fabrics.log
...
# Generate SOC_APF Fabric Qsys scripts 
python3 $TOOLS_PATH/fabric_gen.py --fabric_def soc_apf.txt --fabric_name soc_apf --tcl soc_apf.tcl | tee -a gen_fabrics.log 
...
# Create soc_apf.qsys interconnect fabric
if $QUARTUS_HOME/sopc_builder/bin/qsys-script -qpf=$PROJECT --script=soc_apf.tcl >>gen_fabrics.log 2>>gen_fabrics.log; then
    echo "PASS: STAGE soc_apf.tcl to soc_apf.qsys generation" | tee -a gen_fabrics.log
else
    echo "Error: STAGE soc_apf.qsys generation failed! Check gen_fabrics.log" | tee -a gen_fabrics.log
    exit 1
fi
...
# Generate soc_apf.qsys
if $QUARTUS_HOME/sopc_builder/bin/qsys-generate -syn=VERILOG -sim=VERILOG -qpf=$PROJECT soc_apf.qsys >>gen_fabrics.log 2>>gen_fabrics.log; then
    echo "PASS: STAGE soc_apf.qsys RTL generation" | tee -a gen_fabrics.log
else
    echo "Error: STAGE soc_apf.qsys RTL generation! Check gen_fabrics.log" | tee -a gen_fabrics.log
fi
...
echo "PASS: STAGE APF,BPF and SOC_APF Fabric generation" | tee -a gen_fabrics.log
