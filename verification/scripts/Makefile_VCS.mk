...
VERIF_SCRIPTS_DIR = $(OFS_ROOTDIR)/verification/scripts

TEST_DIR :=  $(shell $(VERDIR)/scripts/create_dir.pl $(VERDIR)/sim/$(TESTNAME) )

VCDFILE = $(VERDIR)/scripts/vpd_dump.key
FSDBFILE = $(VERDIR)/scripts/fsdb_dump.tcl
...
VLOG_OPT += +define+INCLUDE_HSSI +define+INCLUDE_PCIE_SS +define+PCIE_GEN4X16 +define+INCLUDE_PR +define+SIM_MODE_SOC #Enable PCIE SS for Gen4x16 configuration
...
ifdef COV_FUNCTIONAL
    ...
    VLOG_OPT += +define+ENABLE_SOC_HOST_COVERAGE+define+ENABLE_COV_MSG+define+COV_FUNCTIONAL -cm line+cond+fsm+tgl+branch -cm_dir simv.vdb
...
cmplib_adp:
	...
	sh "$(OFS_ROOTDIR)"/ofs-common/scripts/common/sim/gen_sim_files.sh f2000x #Default
...
ifdef AFU_WITH_PIM
	...
	cd $(VERDIR)/sim && vlogan $(VLOG_OPT) +define+FIM_C +define+INCLUDE_PMCI +define+SIM_VIP -F $(SCRIPTS_DIR)/ip_flist.f -F $(SCRIPTS_DIR)/generated_rtl_flist.f -F $(VERIF_SCRIPTS_DIR)/svt_list.f -F $(VERIF_SCRIPTS_DIR)/ver_list.f $(AFU_FLIST_IMPORT)
...
ifndef TEST_DIR
	...
	cd $(VERDIR)/sim && mkdir $(TEST_DIR) && cd $(TEST_DIR) && cp -f ../*.hex . && cp -f $(OFS_ROOTDIR)/ofs-common/src/common/fme_id_rom/fme_id.mif . && cp -f $(VERIF_SCRIPTS_DIR)/fme_id.ver . && cp -f $(VERIF_SCRIPTS_DIR)/recalibration.ver . && cp -f $(OFS_ROOTDIR)/ipss/pmci/pmci_ss_nios_fw.hex . && cp -f $(VERIF_SCRIPTS_DIR)/pmci_ss_nios_fw.ver . | true  && cp -f $(VERDIR)/sim/serdes.firmware.rom . && ../simv $(SIMV_OPT) $(SIMV_OPT_EXTRA)
endif
