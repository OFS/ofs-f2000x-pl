...
/**
 * Abstract:
 * class fme_ras_no_fat_err_seq.svh is executed by fme_ras_no_fat_err_test .
 * 
 * This sequence verifies the RAS no_fatal error regsiter of FME  
            ...
            `uvm_info(get_name(), $psprintf("TEST: READ RAS CAT_FAT_ERROR rdata=%0h",rdata), UVM_LOW)
             ...
             if(rdata ==='h40)begin //check for INJ_waring error 
               ...
               `uvm_info(get_name(), $psprintf("READ RAS NO_FAT_ERROR after clear:- rdata=%0h",rdata), UVM_LOW)
                if(|rdata) begin `uvm_error(get_name(),$psprintf("INJECTED_WARNING ERROR REGISTER is NOT CLEAR by W1C, rdata=%0h",rdata));
                                 //`uvm_info(get_name(), $psprintf("Clear Error regsiter by Writing RAS_ERROR_INJ reg "), UVM_LOW)
                                 // tb_env0.fme_regs.RAS_ERROR_INJ.write(status,'h0);
            	                 // #1us;
                                 // tb_env0.fme_regs.RAS_NOFAT_ERROR.read(status,rdata);
                                 // `ifdef COV tb_env0.fme_regs.RAS_NOFAT_ERROR.cg_vals.sample();`endif
                                 // if(|rdata) `uvm_error(get_name(),$psprintf("INJECTED_WARNING ERROR REGISTER is NOT CLEAR by W1C, rdata=%0h",rdata));
                           end
