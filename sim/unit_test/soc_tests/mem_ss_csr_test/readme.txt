...
To run the test:
 ...
 >>> For Internal Customers:
  ...
  2.) In $OFS_ROOTDIR/ofs-common/scripts/common/sim, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
	  VCS   : sh run_sim.sh TEST=soc_tests/mem_ss_csr_test
	  VCSMX : sh run_sim.sh TEST=soc_tests/mem_ss_csr_test VCSMX=1
	  MSIM : sh run_sim.sh TEST=soc_tests/mem_ss_csr_test MSIM=1
