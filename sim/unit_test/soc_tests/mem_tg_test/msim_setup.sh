...
##################################
vlib work
vlog -mfcu -timescale=1ps/1ps +libext+.v+.sv -lint -sv +initreg+0+top_tb/DUT. +initwire+0 +noinitreg+top_tb/DUT/mem_ss_top. \
 +define+SIM_MODE \
 +define+SIM_MODE_SOC \
 +define+VCD_ON \
 ...
 $MSIM_FILELIST \
 $BASE_AFU_SRC \
 $TB_SRC -work work -l msim_vlog.log -suppress 8386,7033,7061,2388,2732
...
if [ $SKIP_SIM -eq 0 ]; then
  ...
  vsim -c top_tb -suppress 7033,12023,3053,2732 -voptargs="-access=rw+/. -designfile design_2.bin -debug +initreg+0+top_tb/DUT. +initwire+0 +noinitreg+top_tb/DUT/mem_ss_top." -qwavedb=+signal -do "add log -r /* ; run -all; quit -f"
