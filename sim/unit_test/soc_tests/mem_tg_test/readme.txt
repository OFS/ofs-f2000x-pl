...
To run the test:
 ...
 >>> For Internal Customers:
  ...
  2.) In $OFS_ROOTDIR/ofs-common/scripts/common/sim, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
	  VCS   : sh run_sim.sh TEST=host_tests/mem_tg_test
	  VCSMX : sh run_sim.sh TEST=host_tests/mem_tg_test VCSMX=1
	  MSIM : sh run_sim.sh TEST=host_tests/mem_tg_test MSIM=1
