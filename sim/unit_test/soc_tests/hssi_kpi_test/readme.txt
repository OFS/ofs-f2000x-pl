...
To run the test:
  ...
  2.) In <fim> directory, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
        VCS   : sh run_sim.sh TEST=soc_tests/hssi_kpi_test
	     VCSMX : sh run_sim.sh TEST=soc_tests/hssi_kpi_test VCSMX=1
	     MSIM  : sh run_sim.sh TEST=soc_tests/hssi_kpi_test MSIM=1
