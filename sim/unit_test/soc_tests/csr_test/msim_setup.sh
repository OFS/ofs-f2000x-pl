...
##################################
vlib work
vlog -mfcu -timescale=1ps/1ps +libext+.v+.sv -lint -sv +initreg+0 +initwire+0 +initreg+0 +initwire+0 \
 +define+SIM_MODE \
 +define+SIM_MODE_SOC \
 +define+VCD_ON \
 ...
 $MSIM_FILELIST \
 $BASE_AFU_SRC \
 $TB_SRC -work work -l msim_vlog.log -suppress 8386,7033,7061,3053
...
if [ $SKIP_SIM -eq 0 ]; then
  ...
  vsim -c opt -suppress 7033,12023,3053,3053 -do "run -all ; quit -f"
  #vsim -c top_tb -suppress 7033,12023,3053,3053 -voptargs="-access=rw+/. -designfile design_2.bin -debug +initreg+0 +initwire+0" -qwavedb=+signal -do "add log -r /* ; run -all; quit -f"
