...
#
***Test Description***
This is the unit test for HE_MEM attached to the SOC. The test uses HE-MEM to perform memory loopback between FIM and the SOC host.
...
To run the test:
 ...
 >>> For Internal Customers:
  ...
  2.) In $OFS_ROOTDIR/ofs-common/scripts/common/sim, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
	  VCS   : sh run_sim.sh TEST=soc_tests/he_mem_test
	  VCSMX : sh run_sim.sh TEST=soc_tests/he_mem_test VCSMX=1
	  MSIM : sh run_sim.sh TEST=soc_tests/he_mem_test MSIM=1
