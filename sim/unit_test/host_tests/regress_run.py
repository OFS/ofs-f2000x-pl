...
def generate_sim_files():
    ...
    sim_files  = f"sh {sim_files_path} f2000x"
...
def send_email_report():
    ...
    message["Subject"] = f"[F2000X] Unit Test Regression results for OFS AC - Tool:{args.simulator}, Processes:{args.max_parallel_running_process_count}, Package:{args.package}, Rootdir:{rootdir}"
...
def sim_process_normal(index, test, test_dir_top, simulator):
    ...
    test_param = "TEST=" + "/".join(test_dir.split("/")[-2:])
...
def sim_farm_process_normal(index, test, test_dir_top, simulator):
    ...
    if (os.path.exists(test_file)):
        if (simulator == 'vcs'):
            ...
            #build_command_line  = f"arc submit -PE flow/sw/bigmem mem=20000 -- sh {test_file} VCSMX=1"
...
if __name__ == "__main__":
    ...
    rootdir = get_rootdir()
    logger.info(f"      OFS Repo Root Directory is..: {rootdir}")
    if args.email_list:
    ...
    top_test_dir = rootdir + "/sim/unit_test/host_tests"
