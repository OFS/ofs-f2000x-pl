...
To run the test:
  ...
  2.) In $OFS_ROOTDIR/ofs-common/scripts/common/sim, run the following command. Environment variable OFS_ROOTDIR is set in previous step by setup.sh
	  VCS   : sh run_sim.sh TEST=host_tests/pmci_vdm_mctp_mmio_b2b_test
	  VCSMX : sh run_sim.sh TEST=host_tests/pmci_vdm_mctp_mmio_b2b_test VCSMX=1
	  MSIM : sh run_sim.sh TEST=host_tests/pmci_vdm_mctp_mmio_b2b_test MSIM=1
