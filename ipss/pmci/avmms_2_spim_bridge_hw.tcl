...
add_parameter DIR_ADDR_WIDTH INTEGER 20 "AvMM (byte) address width of direct slave access AvMM interface"
set_parameter_property DIR_ADDR_WIDTH DISPLAY_NAME "Direct AvMM Interface Address Width"
...
set_parameter_property DIR_ADDR_WIDTH ALLOWED_RANGES {3:32}
...
proc ip_validate { } {
  ...
  #set dir_baddr  [ get_parameter_value DIR_BASE_ADDR ]
   ...
   set addr_span  [expr {pow(2, $dir_awidth)}]
   ...
   if { $dir_awidth < 16 } {
      send_message Warning "DIR_ADDR_WIDTH (Direct AvMM interface address width) is small. Please make sure if this is sufficient for MCTP over PCIe VDM"
   } elseif { $dir_awidth > 28 } {
      send_message Warning "DIR_ADDR_WIDTH (Direct AvMM interface address width) is large. Please make sure if PXEboot module can handle this"
   }
...
proc ip_elaborate { } {
   ...
   set_port_property avmm_dir_addr width_expr [expr {$dir_awidth - 2}]
