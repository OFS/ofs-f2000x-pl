...
# -- Using the user1 to create the factory image as well since user1/user2/factory are the same for a given image
if which PACSign &> /dev/null ; then
    PACSign FACTORY -y -v -t UPDATE -H openssl_manager  -b ${value_factory} -i ${LOCAL_SCRIPT_DIR}/../output_files/$pacsign_infile_user1 -o ${LOCAL_SCRIPT_DIR}/../output_files/$pacsign_outfile_factory
